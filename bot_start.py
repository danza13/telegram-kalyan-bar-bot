...
CHOOSING, ADDRESS, DATETIME, GUESTS, NAME, PHONE = range(6)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    keyboard = [["Забронювати столик", "Переглянути меню"]]
    reply_markup = ReplyKeyboardMarkup(keyboard, one_time_keyboard=True, resize_keyboard=True)
    await update.message.reply_text("Вітаємо вас в Telegram-бот кальян-бар\n\nТут Ви можете:", reply_markup=reply_markup)
    return CHOOSING

async def view_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    logger.info("Обробник view_menu викликано.")
    await update.message.reply_text("Ось наше меню:")
    await update.message.reply_text("https://gustouapp.com/menu", reply_markup=ReplyKeyboardMarkup([["Повернутись до початку"]], resize_keyboard=True))
...
async def reserve_table(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    keyboard = [ADDRESS_OPTIONS]
    reply_markup = ReplyKeyboardMarkup(keyboard, one_time_keyboard=True, resize_keyboard=True)
    await update.message.reply_text("Виберіть адресу:", reply_markup=reply_markup)
    return ADDRESS
...
async def choose_address(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Запит дати та часу через WebApp
    button = KeyboardButton("Оберіть дату та час", web_app=WebAppInfo(url=WEB_APP_URL))
    reply_markup = ReplyKeyboardMarkup([[button]], resize_keyboard=True, one_time_keyboard=True)
    await update.message.reply_text("Будь ласка, оберіть дату та час, натиснувши кнопку нижче.", reply_markup=reply_markup)
    return DATETIME
...
async def web_app_data_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    logger.info(f"Отримані дані з WebApp: {data}")
    try:
        ...
        return ConversationHandler.END
    context.user_data["datetime"] = info.get("datetime")
...
async def name_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text("Ваш контактний номер телефону для підтвердження бронювання:", reply_markup=reply_markup)
    return PHONE
            ...
            async with session.post(API_URL, json=booking_data) as resp:
                ...
                if resp.status == 200 and response_data.get("status") == "success":
                    await update.message.reply_text(
                        ...
                        reply_markup=ReplyKeyboardMarkup([["Повернутись до початку", "Переглянути меню"]], one_time_keyboard=True, resize_keyboard=True)
                    )
                else:
                    logger.error("API повернув помилку при обробці бронювання.")
                    await update.message.reply_text("Сталася помилка при відправці бронювання. Спробуйте ще раз.", reply_markup=ReplyKeyboardMarkup([["Повернутись до початку"]], resize_keyboard=True))
